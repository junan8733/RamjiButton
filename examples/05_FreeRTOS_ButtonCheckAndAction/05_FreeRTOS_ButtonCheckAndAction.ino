// - 이 예제는 FreeRTOS 환경에서 버튼의 패턴 감지 Task_buttonCheck()와, 감지된 패턴에 따른 동작 수행 Task_buttonExecute()을 병렬로 수행하는 예제입니다.
...
// - This example demonstrates running the button pattern detection task Task_buttonCheck() and the action execution task Task_buttonExecute() concurrently in a FreeRTOS environment.
...
#define USE_FREERTOS // FreeRTOS을 사용하려면 define.
...
// 태스크 : buttonCheck.
void Task_buttonCheck(void *pvParameters) {
  for (;;) {
    // 버튼 각각 독립 이벤트 감지 수행.
    ...
    // UBaseType_t stackLeft = uxTaskGetStackHighWaterMark(NULL); // 현재 실행 중인 태스크의 스택 사용량 정보를 조회.
    // Serial.printf("스택 여유: %u words\r\n", stackLeft); // 남은 스택 워드 수(word 단위) 1 word = 4 bytes (32bit 기준)
    vTaskDelay(pdMS_TO_TICKS(10)); // 10ms마다 반복.
  }
}
// 태스크 : buttonExecute.
void Task_buttonExecute(void *pvParameters) {
  for (;;) {
...
void setup() {
  ...
  // FreeRTOS 태스크 등록.
  xTaskCreate(Task_buttonCheck, "BTCK", 256, NULL, 1, NULL); // 사용 공간 256 words 할당.
  xTaskCreate(Task_buttonExecute, "BTAC", 256, NULL, 1, NULL); // 사용 공간 256 words 할당.
}
